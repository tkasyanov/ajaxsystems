...
*/
  ...
  if ($this->tab=='data') {
   ...
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM ajaxdevicesproperty WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM ajaxdevicesproperty WHERE ajaxdevices_id='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('ajaxdevicesproperty', $properties[$i]);
